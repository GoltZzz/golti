...
const GRILL_ME_INSTRUCTIONS = [
  ...
  '3. The block must be valid JSON inside ```ask-user fences. Always fill "options" with 2-5 realistic answers I might pick. Put the most likely one first and set "recommended": true on it.',
  ...
  '- Top level: { "question": string, "options": array, "allowFreeText": true, "multiSelect": boolean }.',
  '- Always include "allowFreeText": true so I can type my own answer.',
  '- Set "multiSelect": true ONLY when multiple options can be chosen together. Otherwise leave it false/omitted.',
  '- Each option is an object with a "label" (the short button text) and a "description" (one line on what picking it means). Set "recommended": true on your top recommended choice.',
  ...
  '',
  'Skeleton of a correct block. Copy the STRUCTURE only — the placeholder text below is not a topic hint, and your question must come from my actual request:',
  '```ask-user',
  '{ "question": "<the one thing you most need to know>", "options": [{ "label": "<short choice>", "description": "<one line on what picking it means>", "recommended": true }, { "label": "<a genuinely different choice>", "description": "<one line on what picking it means>" }], "allowFreeText": true }',
  '```',
  '',
